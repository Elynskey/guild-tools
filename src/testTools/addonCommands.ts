...
export const ADDON_COMMANDS: AddonCommand[] = [
  ...
  { command: 'calendar', does: "Reads the next 14 days of in-game calendar events and everyone's sign-up status, and saves it for Guild Tools (it also does this quietly a few seconds after login). /reload afterwards so the app sees it.", when: "Before building M+ groups for an event: M+ Comp's Event picker uses it." },
  { command: 'drops', does: "Asks the game's own loot history (the source of drop IDs) what it knows, and saves the answer.", when: 'When wins are captured but carry no drop ID.' },
  ...
  { command: 'synctest', does: 'Pretends another officer just synced the loot you have pending, so you can see your sync button close without a second officer running the addon.', when: 'To check the "button closes for everyone else" behaviour on your own.' },
